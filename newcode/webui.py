...
from src.utils.CreateStructuredJson import createStructureJson
from src.utils.TestCaseutils import testcaseutils, create_test_case_management_tab
from src.utils.testanalyzer_gemini import TestResultAnalyzer
from src.utils.testexecution import get_test_case_description, run_test_case_or_suite
...
from src.utils.utils import update_model_dropdown, get_latest_files, capture_screenshot, remove_screenshot_values
...
async def run_custom_agent(
    ...
    try:
        ...
        with open(history_file, "r") as file:
            history_file2 = json.load(file)

        # Remove screenshot values
        remove_screenshot_values(history_file2)

        with open(history_file, "w") as file:
            json.dump(history_file2, file, indent=4)
...
def create_ui(config, theme_name="Ocean"):
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("🚀 Test Execution", id=5):
                with gr.Row():
            ...
            with gr.TabItem("📊 Results", id=6):
                ...
                run_test_case_button.click(
                    ...
                    outputs=[
                        ...
                        run_test_case_button  # Run button
                    ],
                )
            ...
            create_test_case_management_tab()
...
def write_to_file(filename, data):
    """
    Writes data to a file. If the data is a dictionary or list, it is saved as JSON.
    """
    with open(filename, 'w') as file:
        if isinstance(data, (dict, list)):
            json.dump(data, file, indent=4)  # Save as pretty-printed JSON
        else:
            serialized_data = serialize_custom_agent_brain(data)
            json.dump(serialized_data, file, indent=4)

def serialize_custom_agent_brain(custom_object):
    """
    Converts a CustomAgentBrain object into a JSON-serializable format.
    Replace this with the actual structure of your CustomAgentBrain class.
    """
    if hasattr(custom_object, "__dict__"):
        # If the object has a __dict__ attribute, convert it to a dictionary
        return custom_object.__dict__
    elif isinstance(custom_object, list):
        # If it's a list, recursively serialize each item
        return [serialize_custom_agent_brain(item) for item in custom_object]
    elif isinstance(custom_object, dict):
        # If it's a dictionary, recursively serialize each value
        return {key: serialize_custom_agent_brain(value) for key, value in custom_object.items()}
    else:
...
def write_raw_to_file(filename, data):
    """
    Writes the raw string representation of an object to a file.
    """
    with open(filename, 'w') as file:
        file.write(str(data))
def main():
