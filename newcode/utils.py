...
def remove_screenshot_values(data):
    """
    Recursively removes the value of the 'screenshot' key from a dictionary or list.
    """
    if isinstance(data, dict):
        for key, value in data.items():
            if key == "screenshot":
                data[key] = ""  # Set the value to an empty string
            else:
                remove_screenshot_values(value)  # Recursively process nested dictionaries
    elif isinstance(data, list):
        for item in data:
            remove_screenshot_values(item)